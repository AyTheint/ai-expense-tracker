...
const notifyNotificationsUpdate = () => {
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between gap-3">
                <div>
                    ...
                    <p className="text-sm text-slate-500 mt-1">Set spending limits per category — AI scores each one automatically</p>
                </div>
                <div className="flex items-center gap-2">
                    <button
                        onClick={analyzeAll}
                        disabled={analyzing || budgets.length === 0}
                        className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium border border-emerald-200 bg-emerald-50 text-emerald-700 hover:bg-emerald-100 disabled:opacity-50 disabled:cursor-not-allowed transition"
                    >
            ...
            {loading ? (
                ...
                <EmptyState icon={Target} title="No budgets yet" description="Create a budget to track spending limits." />
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    {budgets.map((b) => {
                        ...
                        return (
                            <div key={b.id} className="bg-white rounded-3xl border border-slate-100 p-6 hover:border-slate-200 transition">
                                {/* Card header */}
                                <div className="flex items-start justify-between mb-5">
                                    <CategoryBadge name={b.category_name} icon={b.category_icon} color={b.category_color} />
                                    <div className="flex items-center gap-1">
                                        <button onClick={() => onEdit(b)} className="p-1.5 hover:bg-slate-100 rounded-md text-slate-500 transition">
                                            <Pencil size={14} />
                                        </button>
                                        <button onClick={() => onDelete(b.id)} className="p-1.5 hover:bg-rose-50 rounded-md text-rose-500 transition">
                                            <Trash2 size={14} />
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div className="mb-3 flex items-baseline justify-between">
                                    ...
                                    <span className="text-sm text-slate-400">of {formatCurrency(total, currency)}</span>
                                </div>
                                ...
                                <div className="mt-2.5 flex items-center justify-between text-xs">
                                    <span className="text-slate-400 capitalize">{b.period} · {pct.toFixed(0)}% used</span>
                                    <span className={over ? 'text-rose-600 font-medium' : 'text-slate-400'}>
                                        {over ? `Over by ${formatCurrency(spent - total, currency)}` : `${formatCurrency(total - spent, currency)} left`}
                                    </span>
                                </div>
                                ...
                                ) : analyzing ? <AnalysisSkeleton /> : null}
                            </div>
                        );
                    })}
                </div>
